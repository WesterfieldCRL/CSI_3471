...
public class Make {
	...
	static class sortHighwayMPG implements  Comparator<ModelSettings>
	{
		@Override
		public int compare(ModelSettings a, ModelSettings b)
		{
			return a.getMpg().avg - b.getMpg().avg;
		}
	}
	...
	public void printNumModelsPerHighwayMPG()
	{
		settingsArr.sort(new sortHighwayMPG());
		String currMPG = String.valueOf(settingsArr.get(0).getMpg().avg);
		int numMPG = 1;
		for (int i = 1; i < settingsArr.size(); i++)
		{
			if (!(Integer.parseInt(currMPG) == settingsArr.get(i).getMpg().avg))
			{
				System.out.println(makeName + ";" + currMPG + ";" + numMPG);
				currMPG = String.valueOf(settingsArr.get(i).getMpg().avg);
				numMPG = 1;
			}
			else
			{
